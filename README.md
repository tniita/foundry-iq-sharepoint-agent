...
- [agent_sharepoint.py](agent_sharepoint.py): Indexed/Remoteの検索バックエンド。Knowledge Baseへの接続とAgent Frameworkの構築を担当します。
...
- ユーザーごとのACLを厳密に保持したまま検索したい場合に限り、[api_server.py](api_server.py) のような中継層で `Authorization: Bearer <token>` を受け、`Bearer ` を除いた生トークンを `user_assertion` として OBO に渡します。
